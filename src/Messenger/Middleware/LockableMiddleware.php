...
class LockableMiddleware implements MiddlewareInterface
{
     ...
     * @throws InvalidArgumentException
     */
    private function getLockConfig(object $message): ?AsLockableMessage
    {
        $cacheKey = md5(self::class.'_'.get_class($message));

        return $this->cache->get($cacheKey, function (ItemInterface $item) use ($message): ?AsLockableMessage {
            ...
            foreach ($reflectionAttributes as $reflectionAttribute) {
                if ($reflectionAttribute->getName() === AsLockableMessage::class) {
